...
namespace Obvs.Extensions
{
    public static class TypeExtensions
    {
        ...
        static ConcurrentDictionary<Type, TypeInfo> infoCache = new ConcurrentDictionary<Type, TypeInfo>();

        public static TypeInfo GetInfo(this Type t)
        {
            TypeInfo tIfo;

            if (!infoCache.TryGetValue(t, out tIfo))
            {
                tIfo = t.GetTypeInfo();
                infoCache.AddOrUpdate(t, tIfo, (e, o) => tIfo);
            }
        ...
        internal static Tuple<MethodInfo, Type, Type>[] GetSubscriberMethods<TCommand, TEvent, TRequest, TResponse>(this Type subscriberType)
        {
            ...
            var methods = publicMethods
                ...
                .Where(m =>
                {
                    ...
                    return (parameterType.GetInfo().IsInterface || parameterType.GetInfo().IsClass) && 
